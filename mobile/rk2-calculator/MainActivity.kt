...
class MainActivity : AppCompatActivity()
{
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val tvExponentation: TextView = findViewById(R.id.tvExponentation)

        tvExponentation.setOnClickListener {
            tvExpression.text = tvExpression.text.toString() + "^"
            tvResult.text = ""
        }
        ...
        val EulerPower: TextView = findViewById(R.id.EulerPower)

        EulerPower.setOnClickListener {
        ...
        tvEquals.setOnClickListener {
            ...
            val optext = tvExpression.text.toString() //Выражение в формате строки
            if (optext != "") {
                try {
                    val expr = ExpressionBuilder(tvExpression.text.toString()).build() //строим выражение
                    ...
                    if (longres.toDouble() == res) { //Если число целое,
                        tvResult.text = longres.toString() //То: Отбрасываем ноль после запятой
                    } else {
                        tvResult.text = res.toString() //Иначе: Сохраняем числа после запятой
                    }
                } catch (e: Exception) { //Если выражение записано некорректно
                    tvResult.text = "Error" //В поле ответа пишем 'Error'
                }
            }
        }
